<template>
  <div class="container mx-auto px-4 py-8">
    ...
    <!-- Debug Info -->
    <div v-if="isDebug" class="mb-4 p-4 bg-gray-100 rounded">
      <p>Current Tracking Number: {{ trackingNumber }}</p>
    ...
    <div v-if="shipmentDetails" class="bg-white rounded-lg shadow p-6">
      ...
      <div class="space-y-6">
        ...
        <div class="grid md:grid-cols-2 gap-4">
          ...
          <div>
            <p class="font-medium">Actual Delivery:</p>
            <p>{{ shipmentDetails.actual_delivery || 'Not delivered yet' }}</p>
          </div>
          <div>
        ...
        <div class="border-t pt-4">
          ...
          <div class="grid md:grid-cols-2 gap-4">
            <div>
              ...
              <p>${{ shipmentDetails.order_details.total_amount }}</p>
            </div>
            <div>
        ...
        <!-- Shipping Updates -->
        <div v-if="shipmentDetails.updates && shipmentDetails.updates.length" class="border-t pt-4">
          <h4 class="text-lg font-semibold mb-3">Shipping Updates</h4>
          <div class="space-y-3">
            <div 
              v-for="update in shipmentDetails.updates" 
              :key="update.id"
              class="border-l-2 border-primary pl-4 py-2"
            >
              <p class="font-medium">{{ update.status }}</p>
              <p class="text-sm text-gray-600">{{ formatDate(update.timestamp) }}</p>
    ...
    async trackOrder() {
      ...
      try {
        ...
        if (error.response?.status === 404) {
          ...
          this.error = 'Please log in to track your order';
        } else {
